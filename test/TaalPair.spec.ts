...
describe('TaalPair', () => {
  ...
  const optimisticTestCases: BigNumber[][] = [
    ...
    [1, 5, 5, '1002004008016032065'] // given amountOut, amountIn = ceiling(amountOut / .9975)
  ].map(a => a.map(n => (typeof n === 'string' ? bigNumberify(n) : expandTo18Decimals(n))))
  optimisticTestCases.forEach((optimisticTestCase, i) => {
    it(`optimistic:${i}`, async () => {
      ...
      await expect(pair.swap(outputAmount.add(1), 0, wallet.address, '0x', overrides)).to.be.revertedWith('Taal: K')
